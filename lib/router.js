...
function compareRoutes (path1, path2) {
  ...
  for ( let i = 0 ; i < path1s.length ; i++ ) {
    ...
    if ( path1s[i].startsWith(':') && path2s[i].startsWith(':') ) {
      continue;
    }

    if ( path1s[i].startsWith(':') ) {
      return 1;
    } else if ( path2s[i].startsWith(':') ) {
      return -1;
    } else {
      return 0;
    }
...
Router.prototype.routes = Router.prototype.middleware = function () {
  ...
  var dispatch = function dispatch(ctx, next) {
    ...
    matchedLayers.sort((a, b) => compareRoutes(a.path, b.path));
